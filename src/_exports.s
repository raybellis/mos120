...
.export CRFS_LOAD	:= $b0
.export CRFS_EXEC	:= $b4
.export CRFS_ERR_PTR	:= $b8
.export CRFS_ERR_PTR_HI	:= $b9
.export CRFS_BLOCK_FLAG	:= $ba
.export CRFS_OPTIONS	:= $bb
.export CRFS_BLK_OFFSET	:= $bc
.export CRFS_BLK_LAST	:= $bd
.export CRFS_CRC_TMP	:= $be
.export CRFS_CRC_TMP_HI	:= $bf
